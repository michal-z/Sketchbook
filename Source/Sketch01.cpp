...
static void Draw()
{
	...
	s_D2dContext->SetTransform(Matrix3x2F::Translation(
		(float)(k_ScreenX / 2 - k_ResolutionX / 2), (float)(k_ScreenY / 2 - k_ResolutionY / 2)));
	
	s_D2dContext->PushAxisAlignedClip(
		RectF(0.0f, 0.0f, (float)k_ResolutionX, (float)k_ResolutionY), D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
#endif
...
static HWND MakeWindow()
{
	...
	RECT rect = { 0, 0, (LONG)k_ScreenX, (LONG)k_ScreenY };
	ShowCursor(FALSE);
#else
	DWORD style = WS_OVERLAPPED | WS_SYSMENU | WS_CAPTION | WS_MINIMIZEBOX;
	RECT rect = { 0, 0, (LONG)k_ResolutionX, (LONG)k_ResolutionY };
	if (!AdjustWindowRect(&rect, style, 0))
		assert(0);
#endif

	HWND hwnd = CreateWindowEx(
		0, k_Name, k_Name, style | WS_VISIBLE,
